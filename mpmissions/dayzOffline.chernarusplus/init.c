...
class CustomMission: MissionServer
{
	...
	override void StartingEquipSetup(PlayerBase player, bool clothesChosen)
	{
		...
		itemClothing = player.GetInventory().CreateInInventory("BaseballCap_CMMG_Black");
		itemClothing = player.GetInventory().CreateInInventory("Jeans_Grey");
		itemClothing = player.GetInventory().CreateInInventory("Hoodie_Red");
		itemClothing = player.GetInventory().CreateInInventory("AthleticShoes_Grey");
		...
		if ( itemClothing )
		{
			...
			itemEnt = itemClothing.GetInventory().CreateInInventory( "BloodTestKit" );
			if ( Class.CastTo( itemBs, itemEnt ) )
				itemBs.SetQuantity( 1 );
